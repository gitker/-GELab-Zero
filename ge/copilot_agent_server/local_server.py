...
def remove_before_think(text):
    # 分割字符串，取最后一部分
    parts = text.split('</think>', 1)  # 只分割一次
    
    if len(parts) > 1:
        return parts[1]  # 返回 </think> 之后的部分
    else:
        return ""  # 如果没有 </think>，返回空字符串或原字符
...
class LocalServer(BaseCopilotServer):
    ...
    def automate_step(self, payload: dict) -> dict:
        ...
        parser_tools = parser.get_tools()
        if parser_tools:
            args["tools"] = parser_tools
        ...
        # Convert response to dict if it's a ChatCompletionMessage object
